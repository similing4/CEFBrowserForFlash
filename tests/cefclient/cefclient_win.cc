...
namespace {
    ...
    bool CreateDirectoryIfNotExists(const std::string& path) {
        std::wstring directoryPath(path.begin(), path.end());
        DWORD attributes = GetFileAttributesW(directoryPath.c_str());
        if (!(attributes != INVALID_FILE_ATTRIBUTES && (attributes & FILE_ATTRIBUTE_DIRECTORY)))
            return CreateDirectoryW(directoryPath.c_str(), nullptr);
        return true;
    }
int RunMain(HINSTANCE hInstance, int nCmdShow) {
  ...
  std::string flash_cache_path = runtime_path + std::string("\\flash_cache");
  std::string user_data_path = runtime_path + std::string("\\user_data");
  CreateDirectoryIfNotExists(flash_cache_path);
  CreateDirectoryIfNotExists(user_data_path);
  context->PopulateSettings(&settings);
  CefString(&settings.locale).FromString("zh-CN");
  CefString(&settings.cache_path).FromString(flash_cache_path.c_str());
  CefString(&settings.user_data_path).FromString(user_data_path.c_str());
  ...
  window_config.with_osr = settings.windowless_rendering_enabled ? true : false;
  window_config.url = "http://web.4399.com/stat/togame.php?target=ddt&server_id=S305";
